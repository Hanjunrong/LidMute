...
public actor SpeakerRecoveryRuntime: SpeakerProtectionApplying, PendingSpeakerRecovering {
    ...
    private func restore(_ originalState: AudioDeviceState, on device: AudioDevice) throws {
        let writableMute = audio.supportsWritableMute(on: device)
        guard writableMute || !originalState.muted else {
        ...
        guard volumeState.volume == originalState.volume else {
            throw SpeakerRecoveryRuntimeError.verificationFailed
        }

        if writableMute {
            try audio.writeMuted(originalState.muted, on: device)
        }

        let restored = try audio.readState(of: device)
        guard restored.volume == originalState.volume,
              (!writableMute || restored.muted == originalState.muted) else {
            throw SpeakerRecoveryRuntimeError.verificationFailed
        }
    }
